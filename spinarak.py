			...
			for subasset in asset['zip']:
				for filepath in glob.glob(tempdirname+subasset['path'], recursive=True):
					if not os.path.isdir(filepath):
						handleAsset(pkg, {'url':filepath, 'type':subasset['type'], 'dest':subasset['dest']+remove_prefix(filepath, tempdirname+subasset['path'].rstrip(".*/"))}, manifest, subasset=True, prepend=prepend+"\t")
